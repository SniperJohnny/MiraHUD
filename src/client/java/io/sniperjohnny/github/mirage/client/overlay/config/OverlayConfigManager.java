...
public class OverlayConfigManager {
    ...
    private static final Path LEGACY_PATH = FabricLoader.getInstance()
            .getConfigDir()
            .resolve(Mirage.MOD_ID + "_overlay.json");
    ...
    private static void migrateIfNeeded() {
        ...
        if (Files.exists(OLD_OVERLAY_PATH) && !Files.exists(CONFIG_PATH)) {
            try {
                ...
                Mirage.LOGGER.info("Migrated overlays.json into overlay/ subfolder.");
            } catch (IOException e) {
                Mirage.LOGGER.warn("Failed to migrate overlays.json to new path", e);
            }
        }
        ...
        if (Files.exists(OLD_PRESETS_DIR) && Files.isDirectory(OLD_PRESETS_DIR)) {
            ...
            if (!Files.exists(newPresetsDir)) {
                try {
                    ...
                    Mirage.LOGGER.info("Migrated presets/ into overlay/ subfolder.");
                } catch (IOException e) {
                    Mirage.LOGGER.warn("Failed to migrate presets/ to new path", e);
                }
            }
        }
        ...
        try {
            ...
            Mirage.LOGGER.warn("Failed to migrate legacy overlay config from {}", LEGACY_PATH, e);
        }
    }
